...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun 13 20:02:37 2023
...
class OperatingPoint2(AeroSandboxObject):
    def __init__(self,
                 p,q,r,
                 velocity: float = 1.,
                 alpha: float = 0.,
    ...
    def compute_rotation_velocity_geometry_axes(self, points):
        ...
        angular_velocity_vector_geometry_axes = jnp.array([
            -self.p,
            self.q,
            -self.r
        ...
        rotation_velocity_geometry_axes = jnp.stack([
            a[1] * b[:, 2] - a[2] * b[:, 1],
            a[2] * b[:, 0] - a[0] * b[:, 2],
            a[0] * b[:, 1] - a[1] * b[:, 0]
        ], axis=1)
