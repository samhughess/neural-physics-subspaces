...
def make_body(file, density, scale):
    ...
    # 1 DOF system
    x0 = jnp.array([c[0], 0, c[1], 0, c[2], 0])
...
class Pendulum:
    ...
    def construct(problem_name):
        ...
        if problem_name == 'pendulum':
            ...
            bodies.append( make_body( os.path.join(".", "data", "Base_pendulum.obj"), 1000, 1.0) )
            bodies.append( make_body( os.path.join(".", "data", "Pendulum_pendulum.obj"), 1000, 1.0) )
    
            joint_list.append( make_joint( 0, -1, bodies, jnp.array([ 0, 0, 0 ]), jnp.array([ 1.0, 0.0, 0.0 ]) ) )
            ...
            system_def['inertia'] = 1/3*0.01*0.05
    ...
    def kinetic_energy_linear(self, system_def, q):
        qR = q.reshape(-1,6,1)
        velocity = jnp.array([qR[-1,1,1], qR[-1,3,1], qR[-1,5,1]])
        vel_mag = jnp.linalg.norm(velocity)
        ke_lin = 1/2*0.01*vel_mag*vel_mag
        return ke_lin

    def potential_energy(self, system_def, q):
        
        qRFull = jnp.concatenate((system_def['fixed_pos'],q)).reshape(-1,6,1)
        ...
        qpos = jnp.array([qR[-1,0,1], qR[-1,2,1], qR[-1,4,1]])
