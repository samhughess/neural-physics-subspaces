...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun 14 15:43:39 2023

@author: samanthahughes
"""
...
class LiftingLine(ImplicitAnalysis):
    ...
    def setup_mesh(self) -> None:
        ...
        for wing_id, wing in enumerate(self.airplane.wings):  # Iterate through wings
            for xsec_a, xsec_b in zip(
                ...
                wing_ids.append(wing_id * jnp.ones(len(faces)))

                if self.spanwise_spacing == 'uniform':
                    y_nondim_vertices = jnp.linspace(0, 1, self.spanwise_resolution + 1)
                elif self.spanwise_spacing == 'cosine':
                    y_nondim_vertices = spacing2.cosspace(0, 1, self.spanwise_resolution + 1)
                else:
                ...
                if wing_section.symmetric:
                    y_nondim = jnp.concatenate([y_nondim, y_nondim])
        ...
        front_left_vertices = jnp.concatenate(front_left_vertices)
        back_left_vertices = jnp.concatenate(back_left_vertices)
        back_right_vertices = jnp.concatenate(back_right_vertices)
        front_right_vertices = jnp.concatenate(front_right_vertices)
        wing_ids = jnp.concatenate(wing_ids)

        ### Compute panel statistics
        diag1 = front_right_vertices - back_left_vertices
        diag2 = front_left_vertices - back_right_vertices
        cross = jnp.cross(diag1, diag2)
        cross_norm = jnp.linalg.norm(cross, axis=1)
        ...
        chords = jnp.linalg.norm(chord_vectors, axis=1)
        ...
        steady_freestream_direction = steady_freestream_velocity / jnp.linalg.norm(steady_freestream_velocity)
        ...
        freestream_influences = jnp.sum(freestream_velocities * normal_directions, axis=1)
    ...
    def get_induced_velocity_at_points(self,
                                       points: jnp.ndarray,
                                       vortex_strengths: jnp.ndarray = None
                                       ) -> jnp.ndarray:
        """
        ...
        u_induced = jnp.sum(u_induced, axis=1)
        v_induced = jnp.sum(v_induced, axis=1)
        w_induced = jnp.sum(w_induced, axis=1)
    ...
    def compute_solution_quantities(self, vortex_strengths: jnp.ndarray) -> Dict:
        ...
        alphas = 90 - jnp.arccosd(
            jnp.sum(velocity_directions * self.normal_directions, axis=1)
        )
        ...
        Vi_cross_li = jnp.cross(velocities, self.vortex_bound_leg, axis=1)
        Vi_cross_li_magnitudes = jnp.linalg.norm(Vi_cross_li, axis=1)
    ...
    def calculate_streamlines(self,
        ...
        if seed_points is None:
            ...
            seed_points_per_panel = jnp.maximum(1, N_streamlines_target // len(left_TE_vertices))

            nondim_node_locations = jnp.linspace(0, 1, seed_points_per_panel + 1)
            nondim_seed_locations = (nondim_node_locations[1:] + nondim_node_locations[:-1]) / 2

            seed_points = jnp.concatenate([
                x * left_TE_vertices + (1 - x) * right_TE_vertices
                for x in nondim_seed_locations
            ])

        streamlines = jnp.empty((len(seed_points), 3, n_steps))
        streamlines[:, :, 0] = seed_points
        for i in range(1, n_steps):
            V = self.get_velocity_at_points(streamlines[:, :, i - 1])
            streamlines[:, :, i] = (
                    streamlines[:, :, i - 1] +
                    length / n_steps * V / tall(jnp.linalg.norm(V, axis=1))
            )
    ...
    def draw(self,
        ...
        if backend == "plotly":
            ...
            faces = tall(range_N) + wide(jnp.array([0, 1, 2, 3]) * N)
    ...
    def _setup_geometry(self):
        ...
        self.beta = jnp.sqrt(1 - self.op_point.mach())
    ...
    def calculate_Vij(self,
        ...
        norm_a = jnp.sqrt(a_x ** 2 + a_y ** 2 + a_z ** 2)
        norm_b = jnp.sqrt(b_x ** 2 + b_y ** 2 + b_z ** 2)
        norm_a_inv = 1 / norm_a
        norm_b_inv = 1 / norm_b
    ...
    def get_induced_velocity_at_point(self, point):
        ...
        Vi = jnp.vstack((Vi_x, Vi_y, Vi_z)).T
    ...
    def draw(self,
        ...
        for fuse_id in range(len(self.airplane.fuselages)):
            ...
            for xsec_id in range(len(fuse.xsecs) - 1):
                ...
                points_1 = jnp.zeros((fuse.circumferential_panels, 3))
                points_2 = jnp.zeros((fuse.circumferential_panels, 3))
                for point_index in range(fuse.circumferential_panels):
                    ...
                    points_1[point_index, :] = jnp.matmul(rot, jnp.array([0, 0, r1]))
                    points_2[point_index, :] = jnp.matmul(rot, jnp.array([0, 0, r2]))
                points_1 = points_1 + jnp.array(xsec_1.xyz_c).reshape(-1)
                points_2 = points_2 + jnp.array(xsec_2.xyz_c).reshape(-1)
